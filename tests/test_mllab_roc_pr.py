...
roc_pr_verdict() — on top of run_baseline_models()'s binary-classification
output. Accuracy alone is misleading on imbalanced classes (which ML Lab
already detects and offers SMOTE for); ROC-AUC and especially the
Precision-Recall curve are the standard remedy.
"""
...
class TestComputeRocPrCurves:
    ...
    def test_multiclass_returns_none(self):
        df = _three_class_df()
        result = run_baseline_models(df, ["x1", "x2"], "target", "classification")
        assert compute_roc_pr_curves(result) is None
